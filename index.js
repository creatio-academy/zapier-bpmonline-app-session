...
const includeSessionHeaders = (request, z, bundle) => {
  if (bundle.authData.cookies && bundle.authData.BPMCSRF) {
    request.headers = request.headers || {};
    request.headers['cookie'] = bundle.authData.cookies;
    request.headers['BPMCSRF'] = bundle.authData.BPMCSRF;
  }
  //console.log("includeSessionHeaders");
...
const sessionRefreshIf401 = (response, z, bundle) => {
  if (bundle.authData.cookies && bundle.authData.BPMCSRF) {
    if (response.status === 401) {
      throw new z.errors.RefreshAuthError('Session auth headers need refreshing.');
    }
  }
  return response;
};
